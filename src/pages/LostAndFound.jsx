...
const LostAndFound = () => {
  ...
  // Local state seeded from mock database alerts with alert_type 'missing_person'
  const missingPersons = reports.length ? reports : [
    {
      id: 1,
      name: 'Sarah Johnson',
      age: 28,
  ...
  const lostItems = itemReports.length ? itemReports : [
    {
      id: 101,
      title: 'iPhone 14 Pro',
      category: 'electronics',
      description: 'Black iPhone 14 Pro with blue case, lost at Central Station',
    ...
    },
    {
      ...
      category: 'documents',
      description: 'Driver\'s license belonging to Maria Rodriguez',
      location: 'Shopping Mall',
      dateFound: '2024-01-14',
      image_url: null,
      status: 'found',
      reportedBy: 'Security Guard',
      contactInfo: 'security@mall.com'
    }
  ]
    ...
    const loadData = async () => {
      try {
        ...
        if (isMounted && personRes && personRes.success) {
          setReports(personRes.data.map(r => ({ ...r, status: r.status || (r.active ? 'missing' : 'found') })))
        }
        ...
        if (isMounted && itemRes && itemRes.success) {
          setItemReports(itemRes.data.map(r => ({ ...r, status: r.status || (r.active ? 'lost' : 'found') })))
        }
  ...
  const submitReport = async (formValues) => {
    ...
    try {
      ...
      if (reportType === 'person') {
        ...
        await executeQuery('INSERT INTO alerts (title, description, severity, location, alert_type, source, image_url, age, gender, last_seen_date) VALUES (?,?,?,?,?,?,?,?,?,?)', 
          [title, description, severity, location, alert_type, source, imageUrl, formValues.age, formValues.gender, formValues.lastSeen])

        // Refresh local list
        const res = await executeQuery('SELECT * FROM alerts WHERE alert_type = ? ORDER BY created_at DESC', ['missing_person'])
        if (res && res.success) setReports(res.data.map(r => ({ ...r, status: r.status || (r.active ? 'missing' : 'found') })))
      } else {
        ...
        if (res && res.success) setItemReports(res.data.map(r => ({ ...r, status: r.status || (r.active ? 'lost' : 'found') })))
      }
